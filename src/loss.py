...
class Seq_Bicriterion(nn.Module):
    def __init__(self, mask_loss):
        super(Seq_Bicriterion, self).__init__()
        self.mask_loss = mask_loss

    def forward(self, output, target):
        seq_output = output['out']
        n_predictions = len(seq_output)
        loss = 0.0
        gamma = 0.8
        for i in range(n_predictions):
            i_weight = gamma**(n_predictions - i - 1)
            mask_loss = self.mask_loss(seq_output[i], target)
            loss += mask_loss * i_weight
        return loss
...
def get_loss(name, loss_weight=None):
    ...
    if name == 'focalloss':
        ...
        return Seq_Bicriterion(mask_loss)
    elif name == 'bi_dice':
        dice_loss = DiceLoss()
